                ...
                while data:
                    ...
                    if j_msg:
                        ...
                        for key in update:
                            ...
                            if ("drone" in key):
                                self.drone = guiDrone(update[key]["x"] + self.x_corrector, self.y_corrector - update[key]["y"], update[key]["z"], self.angle_corrector - update[key]["a"], int(key[-1])+1)
                                self.drone.setup()
                                self.guiObjectsList.append(self.drone)

                        for key in update:
                            if ("drone" in key):
                                self.camera_view = guiCameraView(ci = update[key]["center"], si = int(sqrt(update[key]["size"])), view_id = int(key[-1])+1)
